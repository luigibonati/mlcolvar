...
if mode in [
    ...
    # Load COLVAR generated by PLUMED.
    ...
    if mode in [
        ...
        from mlcolvar.utils.aot import GraphAdapter, load

    if mode == "gnn-kbias-exported":

        # Load AOT-compiled Kolmogorov-bias model.
        ...
        # The AOT model is compiled for a single graph,
        # so evaluate one trajectory frame at a time.
        ...
        z_from_python = torch.stack(z_values)
        q_from_python = torch.stack(q_values)
        kbias_from_python = torch.stack(kbias_values)
        ...
        sys.exit(
            0 if check_z and check_q and check_kbias else 1
        )


    elif mode == "gnn-exported":

        # Load AOT-compiled GNN model.
        model = load("model.pt2")

        # AOT PLUMED interface uses LABEL=gnn.
        cv_from_plumed = torch.Tensor(
