...
def _classification_dataset(
    ...
    for state_index in range(number_of_states):
        class_labels[
            state_index * samples_per_state : (state_index + 1) * samples_per_state
        ] = state_index
        input_data[
            state_index * samples_per_state : (state_index + 1) * samples_per_state,
            state_index % 3,
...
def _regression_dataset(number_of_samples: int = 80) -> DictDataset:
    ...
    target_values = (
        2.0 * input_data[:, 0]
        - 1.5 * input_data[:, 2]
        + 0.1 * random_generator.normal(size=number_of_samples)
    )
...
def test_sparsity_scoring():
    ...
    classifier, _, _ = lasso_classification(
        dataset,
        Cs=[1.0],
        min_features=1,
        print_info=False,
    ...
    sparsity_score = scorer(
        classifier,
        dataset["data"].numpy(),
        dataset["labels"].numpy().astype(int),
    )
    assert np.isfinite(sparsity_score)

    # Case 2: recover accuracy from score for a known configuration.
    recovered_accuracy = scorer.accuracy_from_score(
        score=-2.0,
        num_features=1,
    )
...
def test_lasso_classification_2():
    ...
    for number_of_states in (2, 3):
        ...
        figure, axes = plt.subplots(
            3,
            number_of_models if number_of_models > 1 else 1,
            squeeze=False,
        )
    ...
    dataset_single_regularization = _classification_dataset(
        number_of_states=2,
    ...
    dataset_without_feature_names = _classification_dataset(
        number_of_states=2,
        number_of_samples=40,
    )
...
def test_lasso_regression_2():
    ...
    dataset_with_invalid_target_shape = DictDataset(
        {
            "data": torch.tensor(input_data),
            "target": torch.tensor(invalid_target_values),
        }
    )
    ...
    with pytest.raises(ValueError):
        lasso_regression(
            dataset_with_invalid_target_shape,
            print_info=False,
            plot=False,
        )
...
def test_lasso_print(capsys):
    classification_dataset = _classification_dataset(
        number_of_states=2,
    ...
    single_c_classifier, single_c_features, single_c_coefficients = (
        lasso_classification(
            classification_dataset,
            Cs=[0.1],
            min_features=0,
    ...
    plot_lasso_classification(
        single_c_classifier,
        single_c_features,
        single_c_coefficients,
    )
    assert "Plotting is not available" in capsys.readouterr().out

    single_alpha_regressor, single_alpha_features, single_alpha_coefficients = (
        lasso_regression(
            regression_dataset,
            alphas=[0.1],
            print_info=False,
    ...
    plot_lasso_regression(
        single_alpha_regressor,
        single_alpha_features,
        single_alpha_coefficients,
    )
    assert "Plotting is not available" in capsys.readouterr().out
...
def test_lasso_classification():
    for n_states in [2, 3]:
        ...
        # Correctly create samples for all states.
        X = torch.randn((samples * n_states, 2))

        # create labels
        y = torch.zeros(samples * n_states)
        ...
        for Cs in [40, [0.1], np.logspace(-4, 0, 10)]:
            for min_features in [0, 1]:
                model, features, coeffs = lasso_classification(
                    dataset,
...
def test_lasso_regression():
    ...
    for alphas in [None, [0.1], np.logspace(-4, 0, 10)]:
        model, features, coeffs = lasso_regression(
            dataset,
            alphas=alphas,
            scale_inputs=True,
            print_info=False,
