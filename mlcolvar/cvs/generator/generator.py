...
def test_generator():
    ...
    dataset = DictDataset(
        {"data": ref_pos, "weights": ref_weights, "labels": torch.ones_like(ref_pos)}
    )

    ComputeDistances = PairwiseDistances(
        n_atoms=10, PBC=False, cell=[1, 1, 1], scaled_coords=False
    )
    pos, desc, d_desc_d_pos = compute_descriptors_derivatives(
        dataset, ComputeDistances, n_atoms, separate_boundary_dataset=False
    )
    dataset_desc = DictDataset(
        {
            "data": desc.clone().detach(),
            "weights": dataset["weights"],
            "derivatives": d_desc_d_pos.clone().detach(),
        }
    )
    datamodule = DictModule(
        dataset_desc, lengths=[1.0], random_split=True, shuffle=True
    )
    ...
    for i_atom in range(10):
        friction[3 * i_atom : 3 * i_atom + 3] = torch.tensor(
            [kT / (gamma * masses[i_atom])] * 3, device=device
        )
    ...
    total_loss, loss_var, loss_ortho = model.loss_fn(
        X, q, ref_weights, dataset_desc["derivatives"]
    )

    # Compute eigenfunctions from the learned representation:
    ref_eigenfunctions = torch.tensor(
        [
            [1.5082, 0.2829, 0.9739],
            [1.5256, -1.8986, 0.1828],
            [1.5369, -2.5428, -1.9121],
            [1.4681, 3.4370, -3.2778],
            [1.4635, 0.7059, -1.7016],
        ],
        device=device,
    )
    ref_evals = torch.tensor(
        [-3.6827e-03 + 0.0j, -6.2541e01 + 0.0j, -7.1030e02 + 0.0j], device=device
    )

    ref_evecs = torch.tensor(
        [
            [1.8062e00 + 0.0j, 1.3173e02 + 0.0j, 9.4339e02 + 0.0j],
            [-6.6215e-01 + 0.0j, -2.3322e02 + 0.0j, 1.3187e03 + 0.0j],
            [-8.4665e-01 + 0.0j, 3.3298e02 + 0.0j, 5.2938e02 + 0.0j],
        ],
        device=device,
    )
    ...
    assert torch.allclose(
        evecs.real, ref_evecs.real, atol=1e-1
    )  # Values are very big, so I put a lower tolerance
