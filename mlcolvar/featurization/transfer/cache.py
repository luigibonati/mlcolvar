...
def _precompute_tensor_committor_cache(
    ...
    weights = dataset["weights"].reshape(-1)
    ...
    if not (
        ...
        raise ValueError(
            "`data`, `labels`, `weights`, and `ref_idx` "
            "must have equal lengths."
        )

    original_device = featurizer._model_reference.device
    was_training = featurizer.training
    ...
    try:
        ...
        featurizer.to(original_device)
        featurizer.train(was_training)
...
def _precompute_graph_committor_cache(
    ...
    to contain the same number of atoms.
    """
        ...
        with torch.enable_grad():
            for batch in loader:
                ...
                if not torch.all(counts == counts[0]):
                    raise ValueError(
                        "Graph caching currently requires "
                        "a fixed atom count."
                    )
...
def precompute_committor_cache(
    ...
    device
        Device used for feature and Jacobian computation. Defaults
        to the current featurizer device.
    output_device
        Device on which cached tensors are stored.
    separate_boundary_dataset
        Whether labels <= 1 correspond to boundary-state samples
        and should be excluded from variational Jacobian computation.
    """
    ...
    if isinstance(
        ...
        if descriptor_derivatives is None:
            raise ValueError(
                "`descriptor_derivatives` is required "
                "for tensor-based committor caching."
            )
    ...
    if isinstance(
        ...
        if descriptor_derivatives is not None:
            raise ValueError(
                "`descriptor_derivatives` should not be "
                "provided for graph-based committor caching."
            )
