...
__all__ = ["GeneratorLoss", "compute_eigenfunctions","compute_covariance","forecast_state_occupation" ]
...
class GeneratorLoss(torch.nn.Module):
  ...
  def forward(self, data, output, weights, gradient_descriptors=None):
    return generator_loss( data, output, self.eta, self.alpha, self.friction, self.cell, self.lambdas,  weights, gradient_descriptors)
...
def generator_loss(data, output, eta, alpha, friction, cell, lambdas, weights, gradient_descriptors=None):
    """
    ...
    if gradient_descriptors is None: # If there is no descriptors or a precompute layer
       ...
       gradient_positions = gradient_positions.reshape(-1,output.shape[1],gradient_descriptors.shape[1]*3)* torch.sqrt(friction) 
    if cell is not None:
    ...
    # Compute loss_ortho
    loss_ortho = alpha * (torch.trace((torch.eye(output.shape[1], device=output.device) - cov_X).T @ (torch.eye(output.shape[1], device=output.device) - cov_Y)))
...
def compute_eigenfunctions(input, output, weights, friction, eta, r, cell=None, tikhonov_reg=1e-4, descriptors_derivatives=None):
    """
    ...
    gradient_X = torch.stack([torch.autograd.grad(outputs=output[:,idx].sum(), inputs=input, retain_graph=True, create_graph=True)[0].reshape((-1,d)) for idx in range(r)], dim=2).swapaxes(2,1) 
    
    if descriptors_derivatives is not None: #If there is no precomputing layer
       gradient_positions = torch.einsum("ijk,imkl->ijml", gradient_X, descriptors_derivatives) 
       gradient_positions = gradient_positions.reshape(-1,output.shape[1],descriptors_derivatives.shape[1]*3)* torch.sqrt(friction)
    ...
    operator = torch.linalg.inv(W + tikhonov_reg*torch.eye(output.size(1),device=output.device))@cov_X
