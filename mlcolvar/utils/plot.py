    ...
    * Horizontal bar plot ('barh') only displaying the mean of the gradients
    ...
    if per_class is None:
        ...
        if per_class & (n_results==1):
            raise KeyError('Per class analyis requested but no labels found in the results dictionary. You need to call `sensitivity_analysis` with `per_class`=True. ')
    ...
    for i,label in enumerate(results['sensitivity'].keys()):
        ...
        if mode == 'barh':
            ...
            ax.barh(in_num, score, height=height, color=color, edgecolor="k", hatch=patterns[i], alpha=alpha, label=label)
        elif mode == 'violin':
            ...
            violin_parts = ax.violinplot(grad, positions=in_num, vert=False,showmeans=showmeans,showextrema=False,widths=widths)
            patch_label = _set_violin_attributes(violin_parts,color,alpha=0.5,label=label,zorder=zorder )
            patch_labels.append(patch_label)
            if 'Dataset' in label:
                ax.scatter(y=in_num, x=score, c='dimgrey',s=10,zorder=2)
        elif mode == 'scatter':
            fmt = 'D' if 'Dataset' in label else '.'
            ax.errorbar(y=in_num, x=score, xerr=grad.std(axis=0) ,color=color, fmt=fmt, alpha=0.5, label=label)
        else: 
            raise NotImplementedError('plot mode can be only "barh","violin","scatter".')
